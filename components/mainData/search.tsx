  ...
  return (
    ...
    <div className="w-fit relative">
      <div className="flex items-center">
        ...
        className="p-1 border bg-white border-gray-300 rounded w-full text-md text-indigo-600 outline-none text-base"
        onChange={(e)=> setSearch(e.target.value)}
      />
      <button
        onClick={()=>getAllPokemon(search)}
        className="px-2 py-1.5 text-white bg-gray-900 cursor-pointer
         rounded -ms-14 active:bg-gray-700 hover:bg-gray-700 transition-colors duration-300 text-md"
      >
        search
      </button>
      </div>
        {searched &&
        <div className="absolute text-gray-900 top-9 left-0 right-0 z-30
        flex items-center justify-start gap-2 rounded bg-white/40 backdrop-blur shadow">
